...
mapping = {
    ...
    'Hebrew' : ('EnHe', config.prepare_input_output_dic(config.english_alphabets, config.hebrew_alphabets), 'HeEn', config.prepare_input_output_dic(config.hebrew_alphabets, config.hebrew_alphabets))
}

st.write("# English to Indic Translation")
arch = st.selectbox("Select architecture to translate", ["LSTM", "GRU", "Transformer"], key=1)
...
arch = st.selectbox("Select architecture to translate", ["LSTM", "GRU", "Transformer"], key=2)
